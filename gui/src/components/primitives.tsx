...
export function TextInput({ value, onChange, placeholder, mono }: {
  ...
  return (
    <input
      ...
      className={`w-full rounded-lg border border-border-subtle bg-bg-input px-3.5 py-2.5 text-sm text-text-primary placeholder:text-text-ghost focus:border-border-focus focus:ring-1 focus:ring-border-focus/30 focus:outline-none transition-all duration-200 ${mono ? "font-mono text-[13px]" : ""}`}
    />
  );
}
...
export function NumberInput({ value, onChange, min, max }: {
  ...
  return (
    <input
      ...
      min={min}
      max={max}
      className="w-full rounded-lg border border-border-subtle bg-bg-input px-3.5 py-2.5 text-[13px] font-mono text-text-primary focus:border-border-focus focus:ring-1 focus:ring-border-focus/30 focus:outline-none transition-all duration-200 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
    />
  );
}
...
export function ToggleRow({ label, desc, checked, onChange }: {
  ...
  return (
    <div className="group flex items-center justify-between gap-3 py-3 rounded-lg transition-colors duration-200 hover:bg-bg-hover/50">
